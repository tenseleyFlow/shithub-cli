...
// TestDeferredStubsExit walks each stub subcommand and confirms the
// deferred exit-code+message contract holds.
func TestDeferredStubsExit(t *testing.T) {
	tf := cmdutiltest.New(t)
	parent := NewCmd(tf.Factory)
	stubs := []string{"enable", "disable"}

	for _, name := range stubs {
		t.Run(name, func(t *testing.T) {
			sub := findSubcommand(parent.Commands(), name)
			if sub == nil {
				t.Fatalf("subcommand %q not registered on parent", name)
			}
...
// drift: list/view/run plus the two deferred stubs.
