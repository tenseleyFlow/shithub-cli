...
// Package delete implements `shithub secret delete <name>`.
package delete
...
func NewCmd(f *cmdutil.Factory) *cobra.Command {
	...
	cmd.Flags().StringVarP(&opts.Org, "org", "o", "", "delete an org-level secret")
...
func Run(ctx context.Context, opts *options) error {
	if opts.App != "" && opts.App != "actions" {
		return fmt.Errorf("secret delete: only --app actions is supported (got %q)", opts.App)
	}
	...
	if opts.Org != "" {
		...
		if err := secrets.NewClient(client).DeleteOrgSecret(ctx, opts.Org, opts.Name); err != nil {
			return err
		}
		fmt.Fprintf(opts.IO.ErrOut, "%s Deleted secret %s from org %s\n",
	...
	if err := secrets.NewClient(client).DeleteRepoSecret(ctx, ref.Owner, ref.Name, opts.Name); err != nil {
		return err
	}
	fmt.Fprintf(opts.IO.ErrOut, "%s Deleted secret %s from %s/%s\n",
